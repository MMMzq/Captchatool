...
def demo1():
    ...
    # 创建数据
    if not os.path.exists('data/demo1'):
...
def demo2():
    ...
    if not os.path.exists(model_path):
        os.makedirs(model_path)
    # 创建数据
    if not os.path.exists('data/demo2'):
        generate_todir(charset,4,train_path,160,60,4000)
        generate_todir(charset,4,test_path,160,60,400)
    ct=Code_tool(charset,model_path,train_path,test_path,label_resolve)
    ct.train(10,0.5,0.75,True)
    fn=os.listdir(test_path)[0]
    label=label_resolve(fn)
    code=ct.infer_file(test_path+fn)
    print('正确为{}\t推测为{}'.format(label,code))
...
def demo3():
    ...
    if not os.path.exists(model_path):
        os.makedirs(model_path)
    if not os.path.exists('data/demo3'):
        generate_todir(charset,4,train_path,160,60,6000)
        generate_todir(charset,4,test_path,160,60,600)
    ct=Code_tool(charset,model_path,train_path,test_path,label_resolve)
    ct.train(10,0.5,0.75,True)
    fn=os.listdir(test_path)[0]
    label=label_resolve(fn)
...
def demo5():
    ...
    with open(test_path+fn,'rb') as f:
        ...
        print('正确为{}\t推测为{}'.format(label,code))
    # 从str对象读取图片来推测
    with open(test_path+fn) as f:
        s=f.read()
        code=ct.infer_string(b)
        label=label_resolve(fn)
        print('正确为{}\t推测为{}'.format(label,code))
# 取消注释来运行其他demo
